...
EXPERIMENTS_PREFIX = "experiment-"
EXPERIMENT_NAME_FORMAT = EXPERIMENTS_PREFIX + "{:04d}"


def generate_parameters_for_all_experiments() -> Iterator[Dict[str, Any]]:
    """
    Generate all possible combinations of parameters for the experiments. The study
    parameters definition (stored in PARAMETERS_FILEPATH) is a nested
    dictionary, the first level of which is the filename of the template to be
    used to generate the experiment. The subsequent level in the dictionary are
    the parameters that will be passed to the Jinja2 template.
...
def generate_experiments():
    ...
    for i, experiment_parameters in enumerate(
        ...
        with open(experiment_path / "experiment_parameters.yaml", "w") as f:
            # add a comment at the top of the file with the parameters used saying the file was generated and for what purpose
            f.write("# This file was generated by generate_experiments.py on\n")
            f.write(f"# {datetime.datetime.now().isoformat()}\n")
            f.write("# with the following parameters:\n")
